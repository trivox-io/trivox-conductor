...
def import_adapter_from_descriptor(descriptor: PluginDescriptor, pkg_root: str):
    """
    ...
    :type pkg_root: str
    """
    mod_path = f"{pkg_root}.plugins.{descriptor.name}.{descriptor.module}".replace("-", "_")
    try:
        mod = importlib.import_module(mod_path)
        return getattr(mod, descriptor.clazz)
    except (ImportError, AttributeError) as e:
        raise ImportError(f"Failed to import {descriptor.clazz} from {mod_path}") from e
